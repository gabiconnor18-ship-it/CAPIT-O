...
// Supabase DB service
import * as db from "./src/supabaseService";
...
// ----------------------------------------------------
// RATE LIMITER MIDDLEWARE (Intrusion/Abuse Prevention)
// ----------------------------------------------------
...
const RATE_LIMIT_MAX_REQUESTS = 100; // max 100 requests per 15 minutes per IP
...
app.post("/api/auth/login", async (req, res) => {
  try {
    ...
    if (!db.verifyPassword(password, customer.password)) {
...
app.post("/api/auth/admin-login", async (req, res) => {
  try {
    ...
    if (!admin) {
      return res.status(401).json({ error: "Administrador não cadastrado com este e-mail." });
    }
    if (!db.verifyPassword(password, admin.password)) {
    ...
    res.status(500).json({ error: error.message || "Erro ao efetuar login de administrador." });
  }
});
...
app.post("/api/auth/admin-register", async (req, res) => {
  try {
    ...
    if (!name || !email || !password) {
      return res.status(400).json({ error: "Nome, e-mail e senha de administrador são obrigatórios." });
    }

    const existing = await db.getAdminByEmail(email);
    if (existing) {
      return res.status(400).json({ error: "E-mail de administrador já cadastrado." });
    }
    ...
    const created = await db.addAdmin(newAdmin);
    const adminProfile = {
      id: created.id,
      name: created.name,
      email: created.email
    ...
    res.status(500).json({ error: error.message || "Erro ao registrar administrador." });
  }
});

// List customers (For admin interface management!)
app.get("/api/customers", async (req, res) => {
  try {
    const customers = await db.getCustomers();
    // Return without password mapping snake to camel case
    const safeCustomers = customers.map(({ password, loyalty_points, ...c }) => ({
      ...c,
...
// List admins (For admin interface management!)
