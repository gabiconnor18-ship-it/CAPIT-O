...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const loginAdmin = async (email: string, password: string): Promise<boolean> => {
    try {
      ...
      if (!res.ok) {
        const err = await res.json();
        throw new Error(err.error || 'Erro no login de admin');
      }
      const adminData = await res.json();
      setIsLogged(true);
      setIsAdmin(true);
      addNotification("Acesso Administrativo ⚙️", `Olá, administrador ${adminData.name}!`);
      return true;
    } catch (err: any) {
      console.error(err);
      addNotification("Acesso Negado ❌", err.message || "E-mail ou senha administrativa inválidos.");
      throw err;
    }
  };
